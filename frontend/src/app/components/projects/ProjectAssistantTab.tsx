...
// Assistant tab inside a project page — chat list view.
//
// Extracted from ProjectPage.tsx (Step D of the upstream f39f175
// PR #64 refactor). Pure presentation: all state and side-effect
// callbacks come in as props from the orchestrator. Behaviour is
// identical to the previous inline rendering — the only change is
// that delete/rename/open are now callbacks rather than inline
// closures over the parent's state.
                    ...
                    {filteredChats.map((chat) => (
                        ...
                        >
                            ...
                            <div className={`sticky left-8 z-[60] ${NAME_COL_W} p-2 ${selectedChatIds.includes(chat.id) ? "bg-gray-50" : "bg-white"} group-hover:bg-gray-50`}>
                                {renamingChatId === chat.id ? (
                                    <input
                                        ...
                                        onKeyDown={(e) => {
                                            if (e.key === "Enter") void submitChatRename(chat.id);
                            ...
                            <div className="w-8 shrink-0 flex justify-end" onClick={(e) => e.stopPropagation()}>
                                <RowActions
                                    onRename={() => {
                                        if (currentUserId && chat.user_id !== currentUserId) {
