...
// Reviews tab inside a project page — tabular-review list view.
//
// Extracted from ProjectPage.tsx (Step E of the upstream f39f175
// PR #64 refactor). Same shape as ProjectAssistantTab: pure presentation,
// state + side-effects come in as props from the orchestrator.
                            ...
                            <div className={`sticky left-8 z-[60] ${NAME_COL_W} p-2 ${selectedReviewIds.includes(review.id) ? "bg-gray-50" : "bg-white"} group-hover:bg-gray-50`}>
                                {renamingReviewId === review.id ? (
                                    <input
                                        ...
                                        onKeyDown={(e) => {
                                            if (e.key === "Enter") void submitReviewRename(review.id);
                                            if (e.key === "Escape") setRenamingReviewId(null);
                                        }}
                                        onBlur={() => void submitReviewRename(review.id)}
                                        onClick={(e) => e.stopPropagation()}
                                ...
                                <RowActions
                                    onRename={() => {
                                        if (currentUserId && review.user_id !== currentUserId) {
                                            onOwnerOnlyAction("rename this tabular review");
                                            return;
                                        }
