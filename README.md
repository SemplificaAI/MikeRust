...
| **Audio transcription (Phase 1: file upload)** — whisper.cpp via `whisper-rs` behind the `audio-transcription` feature; symphonia → 16 kHz mono PCM → whisper; `[T MM:SS]` segment markers ride the existing chunker/citation path; `GET /document/:id/transcript` exposes joined body + structured segments; `AudioView.svelte` plays the file + clickable transcript that seeks the player; citation pill resolution seeks straight to the cited segment via `start_ms`. First-use GGML download → auto-bootstrapped from HuggingFace into `%USERPROFILE%/mikerust-data/whisper/`; `/sync/whisper-status` reports progress | ⚠️ **EXPERIMENTAL — not yet end-to-end tested.** Code compiles without the feature; with the feature, the vendored whisper.cpp compile requires LLVM-ARM64 libclang on Windows ARM64 (see [`docs/whisper-mike-plan.md`](docs/whisper-mike-plan.md) §2.1) and no real-file smoke test has run yet. Promote to ✅ once at least one mp3 / wav round-trips upload → transcribe → cited-segment seek on a developer machine. |
| Audio transcription — **Phase 2** (microphone recording + input-device & volume modal) | 🔲 deferred — design sketched in `docs/whisper-mike-plan.md` §1 and §7; entry point is the same `transcribe_audio` once a PCM stream is fed in. Includes the Tauri webview `getUserMedia` permission flow, MediaRecorder→PCM transcode, device picker and volume meter |
| Audio transcription — **Phase 3** (diarization + VAD-chunked parallel transcription + speaker labels) | 🔲 deferred — improves long-hearing accuracy and read-time; not on the immediate roadmap |
